...
REFERENCE CODE
  Pattern: # symbol followed by letter(s) and digits. e.g. #c90282, #c91049.
...
PRICE
  ...
  For ranges like "500/600" or "$500 / $600 with 2 internals" → HIGHER value (600).
  0 if not found.
...
Assign each physical TV to EXACTLY ONE primary type.
  ...
  G) standard_mounting ← DEFAULT
     All TVs that don't qualify for A–F.
...
BRACKET TYPE (one per TV):
 ...
 6C. SOUNDBAR ─
  soundbar_installation → soundbar_mount_s: +1 per soundbar count
  Default is soundbar_mount_s (small mount used in 80% of cases).
  Only use soundbar_mount_b if explicitly stated "big soundbar" or "large soundbar".
...

{
  ...
  "properties": {
    ...
    "status": {
      "type": "string",
      "enum": ["booked", "warranty", "cancelled", "rescheduled", "pending"]
    },
    "price": {
      "type": "number"
    },
  ...
  "required": ["is_valid_job", "price", "job_description", "technical_tags", "expected_services", "expected_materials", "city", "street"]
}`;
